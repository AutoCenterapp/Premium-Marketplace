...
@Component({
  ...
  selector: 'app-apply-online-iframe',
  templateUrl: './apply-online-iframe.component.html',
  styleUrls: ['./apply-online-iframe.component.css']
})
export class ApplyOnlineIframeComponent implements OnInit {
  ...
  overlay = false;
  data: Select2Data = [];
  select2Placeholder: any="Select Vehicle";
  ...
  getVehciles(id:any)
  {
    ...
    this.backendService.getAllVehicles(`filter[dealership_id]=${id}`).subscribe((response: any) => {
      if (response.status == 200 && response.body) {
  ...
  update( event: Select2UpdateEvent<Select2UpdateValue>) {
    if(event.value=="undefined")
    {
    this.applyOnlineForm.controls.vehicle_id.setValue("");
    }
    else
    {
      // @ts-ignore
      this.applyOnlineForm.controls.vehicle_id.setValue(event.value);
    }
  }
  vehiclesSearch(event: Select2SearchEvent<Select2UpdateValue>) {
    console.log(event.search);
    this.backendService.getVehiclesAgainstSelectedKeywordApply(`filter[dealership_id]=${this.dealerShipId}&filter[title][like]=${event.search}`).subscribe((response: any) => {
      if (response.status == 200 && response.body) {
        this.data = [] as Select2Data;
        response.body.forEach((data: any) => {
          this.data.push({
            value: data?.id,
            label: data?.title,
  ...
  getSpecificDealerShip() {
    this.backendService.getSpecificDealerShip(this.dealershipSlug).subscribe((response: any) => {
      if (response.status == 200 && response.body) {
        ...
        this.getVehciles(response.body[0]?.id);
      }
      this.getUserProfile()
    });
  }
  ...
  onSubmitApplyOnlineForm() {
    ...
    if (this.checkAuthService.IsLogin() == false) {
      ...
      let Password = this.randomString(10, '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ')
      ...
      this.signupForm.controls.confirmPassword.setValue(Password);
