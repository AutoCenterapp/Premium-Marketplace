...
export class MarketplaceComponent implements OnInit, OnDestroy {
  ...
  constructor(private titleService: Title, private modalService: NgbModal, private backendService: BackendService) {
  }
  ...
  ngOnInit(): void {
    this.titleService.setTitle('Auto Center - Marketplace')
    this.modelIdCheck();
  }
  ...
  getVehicles() {
    ...
    this.backendService.getVehicles(this.carListingPageSize, this.pageNumber, '').subscribe((response: any) => {
  ...
  getVehiclesAgainstModel() {
    this.backendService.getVehiclesAgainstModel(this.carListingPageSize, this.pageNumber, Number(this.searchedVehicle.id)).subscribe((response: any) => {
      if (response.status == 200 && response.body && response.body.length > 0) {
        this.totalCount = response.headers.get('X-Pagination-Total-Count')
        response.body.forEach((data: any) => {
          this.vehicles.push(data);
  ...
  getVehiclesAgainstSelectedVehicleType() {
    this.backendService.getVehiclesAgainstSelectedVehicleType(this.carListingPageSize, this.pageNumberSelectedVehicleTypeSearch, Number(this.selectedVehicleType.id)).subscribe((response: any) => {
      if (response.status == 200 && response.body && response.body.length > 0) {
        this.totalCount = response.headers.get('X-Pagination-Total-Count')
        response.body.forEach((data: any) => {
          this.vehicles.push(data);
  ...
  getVehiclesAgainstSelectedKeyword() {
    this.loadingData = true;
    this.backendService.getVehiclesAgainstSelectedKeyword(this.carListingPageSize, this.pageNumberSelectedKeywordSearch, this.searchKeyword).subscribe((response: any) => {
      this.loadingData = false;
      if (response.status == 200 && response.body && response.body.length > 0) {
        this.totalCount = response.headers.get('X-Pagination-Total-Count');
        if (this.vehicles.length >= this.totalCount) {
  ...
  getVehiclesAgainstAdvanceSearch() {
    this.backendService.advancedSearch(this.advanceSearchResults.filters, this.pageNumberAdvanceSearch).subscribe((response: any) => {
      if (response.status == 200 && response.body && response.body.length > 0) {
        this.totalCount = response.headers.get('X-Pagination-Total-Count')
        response.body.forEach((data: any) => {
          this.vehicles.push(data);
